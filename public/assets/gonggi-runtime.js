(function () {
  const ID = 'gonggi';
  const RK = 'hanpan-gonggi-records-v4';
  const PK = 'hanpan-gonggi-prefs-v4';
  const P = {
    easy: { label: '쉬움', flight: 2.5, catchStart: .65, catchEnd: .96, pickupStart: .06, pickupEnd: .75, lives: 4 },
    normal: { label: '보통', flight: 2.05, catchStart: .71, catchEnd: .93, pickupStart: .08, pickupEnd: .71, lives: 3 },
    hard: { label: '어려움', flight: 1.68, catchStart: .76, catchEnd: .91, pickupStart: .10, pickupEnd: .67, lives: 2 }
  };
  const stages = [
  ...
  const spotPool = [
    { x: 8, y: 58 }, { x: 31, y: 18 }, { x: 55, y: 54 }, { x: 78, y: 12 },
    { x: 18, y: 12 }, { x: 42, y: 62 }, { x: 66, y: 22 }, { x: 82, y: 60 }
  ];
  ...
  function randomLayout() {
    const shuffled = spotPool.slice().sort(() => Math.random() - .5).slice(0, 4);
    return shuffled.map((p) => ({ x: p.x + (Math.random() - .5) * 5, y: p.y + (Math.random() - .5) * 5 }));
  }

  function mount(surface) {
    if (!surface || surface.dataset.gameId !== ID || surface.querySelector('.gg4')) return;
    ...
    let flying = false, flightStart = 0, progress = 0, picked = new Set(), remaining = freshStones(), layout = randomLayout(), ready = true, flipStep = 0, backhand = 0;
    ...
    surface.innerHTML = `
      <style>
        .gg4{max-width:700px;margin:auto;display:grid;gap:12px;text-align:center}
        .gg4-hud,.gg4-actions{display:flex;gap:8px;justify-content:center;align-items:center;flex-wrap:wrap}
        .gg4-hud span,.gg4-hud strong,.gg4-records span{padding:7px 10px;border-radius:999px;background:rgba(29,36,51,.07)}
        .gg4-board{position:relative;min-height:390px;border-radius:22px;overflow:hidden;background:linear-gradient(#f6eedc 0 70%,#d6b47b 70%);border:1px solid #d1bf9d;touch-action:manipulation}
        .gg4-air{position:absolute;left:50%;bottom:78px;width:42px;height:42px;border-radius:50%;transform:translate(-50%,50%);background:radial-gradient(circle at 35% 30%,#fff,#89a9cd 55%,#476a93);box-shadow:0 8px 12px rgba(0,0,0,.18);display:none;z-index:4}
        ...
        .gg4-hand{position:absolute;left:50%;bottom:30px;width:150px;height:52px;transform:translateX(-50%);border-radius:70% 70% 35% 35%;background:#f0b98f;border:3px solid #9a694c;z-index:2}.gg4-hand:after{content:"손";position:absolute;left:50%;top:50%;transform:translate(-50%,-50%);font-weight:900;color:#704932}
        .gg4-ground{position:absolute;left:8%;right:8%;bottom:82px;height:150px;z-index:3}
        .gg4-stone{position:absolute;width:64px;height:64px;transform:translate(-50%,-50%);border-radius:50%;border:3px solid #53749a;background:radial-gradient(circle at 35% 30%,#fff,#8fafcf 58%,#55779e);box-shadow:0 7px 10px rgba(0,0,0,.14);cursor:pointer;transition:transform .12s,opacity .12s;font-weight:900;color:#254563}
        .gg4-stone.picked{opacity:.18;transform:translate(-50%,-75%) scale(.8);pointer-events:none}.gg4-stone.removed{opacity:0;pointer-events:none}.gg4-stone:disabled{cursor:default}.gg4-stone.late{filter:grayscale(.45);opacity:.55}
        .gg4-prompt{position:absolute;left:50%;top:18px;transform:translateX(-50%);padding:9px 14px;border-radius:999px;background:#fffdf2e8;font-weight:900;white-space:nowrap;z-index:5}
        .gg4-stage-name{font-size:1.08rem;font-weight:900}.gg4-help{font-size:.92rem;color:#566070}.gg4 button,.gg4 select{min-height:44px}
        @media(max-width:600px){.gg4-board{min-height:350px}.gg4-ground{left:4%;right:4%;height:140px}.gg4-stone{width:54px;height:54px}.gg4-actions .button{flex:1 1 125px}}
      </style>
      <div class="gg4">
        <div class="gg4-hud"><strong data-v="score">0점</strong><span data-v="stage">1/5단계</span><span data-v="lives">목숨 3</span><span data-v="combo">콤보 0</span></div>
        <div class="gg4-actions"><label>난이도 <select data-v="diff"><option value="easy">쉬움</option><option value="normal">보통</option><option value="hard">어려움</option></select></label><button class="button secondary" data-act="new" type="button">새 도전</button></div>
        <div class="gg4-stage-name" data-v="stageName"></div>
        <div class="gg4-board" aria-label="공깃돌을 던지고 공중에 떠 있는 동안 바닥 돌을 주운 뒤 다시 받는 공기놀이 게임 영역"><div class="gg4-prompt"></div><div class="gg4-air" aria-hidden="true"></div><div class="gg4-ground"></div><div class="gg4-hand" aria-hidden="true"></div></div>
        <div class="gg4-actions"><button class="button primary" data-act="toss" type="button">공깃돌 던지기 · Enter</button><button class="button primary" data-act="catch" type="button">받기 · Space</button></div>
        <div class="gg4-records"></div><p class="gg4-help">던진 돌이 올라가 있는 동안 흩어진 바닥 돌을 필요한 개수만큼 직접 고르세요. 너무 일찍 또는 너무 늦게 줍거나, 필요한 개수보다 더 집으면 실수입니다.</p>
      </div>`;

    const $ = (q) => surface.querySelector(q);
    const ground = $('.gg4-ground'), air = $('.gg4-air'), prompt = $('.gg4-prompt'), tossBtn = $('[data-act="toss"]'), catchBtn = $('[data-act="catch"]'), sel = $('[data-v="diff"]');
    sel.value = diff;
    ...
    function renderStones() {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        b.addEventListener('click', () => pickStone(i)); ground.appendChild(b);
      }
      ground.style.display = stage === 4 ? 'none' : 'block';
    }
    function render() {
      const r = record();
      ...
      $('.gg4-records').innerHTML = `<span>${P[diff].label} 최고 ${r.bestScore}점</span><span>최고 단계 ${r.bestStage}/5</span><span>최고 콤보 ${r.bestCombo}</span><span>완주 ${r.clears}회</span>`;
    ...
    function setPrompt() {
      ...
      if (!flying) prompt.textContent = `${need}알을 줍는 차례 · 바닥 ${remaining.size}알`;
      else if (progress < P[diff].pickupStart) prompt.textContent = '던진 돌이 조금 올라가면 바닥 돌을 주우세요';
      else if (progress <= P[diff].pickupEnd) prompt.textContent = `${need}알 줍기 · 현재 ${picked.size}/${need} · 바닥 ${remaining.size}알`;
      else prompt.textContent = '줍기 시간 종료 · 이제 받기 준비';
    }

    function reset() {
      stage = 0; turn = 0; lives = P[diff].lives; score = 0; combo = 0; bestCombo = 0; flying = false; picked = new Set(); remaining = freshStones(); layout = randomLayout(); ready = true; locked = false; flipStep = 0; backhand = 0; progress = 0;
      record().runs++; save(RK, records); air.style.display = 'none'; air.classList.remove('multi'); setPrompt(); render(); announce(`${P[diff].label} 공기놀이 시작. 던지기 → 줍기 → 받기 순서를 실제 타이밍에 맞춰 진행하세요.`);
    }
    ...
    function toss() {
      ...
      announce(stage === 4 ? (flipStep === 0 ? '다섯 알을 띄웠습니다. 꼭대기에서 손등 받기!' : '손등의 돌을 띄웠습니다. 내려올 때 뒤집어 받기!') : `${currentGroup()}알을 공중 시간 안에 주운 뒤 내려오는 돌을 받으세요.`);
    }

    function pickStone(index) {
      if (!flying || stage === 4 || locked || picked.has(index) || !remaining.has(index)) return;
      const cfg = P[diff];
      if (progress < cfg.pickupStart) { fail('공깃돌이 충분히 올라가기 전에 손을 뻗었습니다.'); return; }
      if (progress > cfg.pickupEnd) { fail('바닥 돌을 줍는 타이밍이 너무 늦었습니다.'); return; }
      const need = currentGroup();
      picked.add(index);
    ...
    function fail(reason) {
      ...
      setTimeout(() => { locked = false; ready = true; setPrompt(); render(); }, 620); render();
    }

    function succeedTurn(timingBonus) {
      flying = false; air.style.display = 'none'; combo++; bestCombo = Math.max(bestCombo, combo); score += 55 + stage * 25 + timingBonus + combo * 4;
      picked.forEach((index) => remaining.delete(index));
      turn++; picked = new Set(); locked = true;
      if (turn >= stages[stage].groups.length) {
        score += 125 + stage * 30; stage++; turn = 0; remaining = freshStones(); layout = randomLayout(); combo += 2; bestCombo = Math.max(bestCombo, combo);
      }
      ...
      setTimeout(() => { locked = false; ready = true; flipStep = 0; setPrompt(); render(); }, 580); render();
    }
    ...
    function catchStone() {
      ...
      if (stage === 4) {
        if (flipStep === 0) {
          ...
          setTimeout(() => { locked = false; ready = true; setPrompt(); render(); }, 500); render(); return;
        }
