(function () {
  ...
  const PREF_KEY = "hanpan-match3-prefs-v2";
  const rows = 7;
  const cols = 7;
  const colorCount = 6;
  const symbols = ["●", "◆", "▲", "■", "★", "✦"];
  ...
  function writeJson(key, value) {
    try { localStorage.setItem(key, JSON.stringify(value)); } catch (error) { /* storage optional */ }
  }
  ...
  function cloneBoard(board) { return board.map(function (item) { return item ? tile(item.c, item.s) : null; }); }
  ...
  function swap(board, a, b) {
    const temp = board[a]; board[a] = board[b]; board[b] = temp;
  }

  function specialSwapWorks(board, a, b) {
    const ta = board[a], tb = board[b];
    return Boolean((ta && ta.s === "rainbow") || (tb && tb.s === "rainbow"));
  }
  ...
  function makeBoardFallback() {
    ...
    board[indexOf(3, 2)] = tile(0); board[indexOf(3, 3)] = tile(1); board[indexOf(3, 4)] = tile(0); board[indexOf(2, 3)] = tile(0);
    return board;
  }
  ...
  function addStyle() {
    ...
    style.textContent = `
      .match3-runtime-game{--m3-gap:6px}
      ...
      .match3-board{display:grid;grid-template-columns:repeat(7,1fr);gap:var(--m3-gap);width:min(100%,500px);aspect-ratio:1;margin:0 auto;padding:8px;border:2px solid rgba(29,36,51,.18);border-radius:16px;background:#e9e2d4;touch-action:none;user-select:none}
      .match3-cell{position:relative;min-width:0;min-height:0;border:1px solid rgba(29,36,51,.22);border-radius:12px;background:#fffaf0;font:800 clamp(1rem,4.5vw,1.8rem)/1 system-ui,sans-serif;cursor:pointer;transition:transform .12s ease,box-shadow .12s ease}
      ...
      .match3-cell.is-clearing{transform:scale(.25);opacity:.25}
  ...
  function setup(surface) {
    ...
    let saved = readJson(SAVE_KEY, null);
    let record = Object.assign({ bestScore: 0, bestStage: 1, clears: 0, bestCombo: 0 }, readJson(RECORD_KEY, {}));
    let stage = 1;
    let board = [];
    let score = 0;
    let movesLeft = 0;
    ...
    const hudValues = hudLabels.map(function (label) {
      const span = document.createElement("span"); const b = document.createElement("b"); b.textContent = "-"; const small = document.createElement("small"); small.textContent = label; span.append(b, small); hud.appendChild(span); return b;
    });
    ...
    function saveGame() {
      if (finished) return;
      writeJson(SAVE_KEY, { version: 2, stage: stage, board: board, score: score, movesLeft: movesLeft, colorCleared: colorCleared, comboBest: comboBest });
    }
    function validSaved(value) {
      return value && value.version === 2 && Number.isInteger(value.stage) && value.stage >= 1 && Array.isArray(value.board) && value.board.length === rows * cols && value.board.every(function (item) { return item && Number.isInteger(item.c) && item.c >= 0 && item.c < colorCount && [null,"h","v","rainbow"].includes(item.s || null); });
    }
    ...
    function render(chain) {
      ...
      board.forEach(function (item, index) {
        const button = document.createElement("button"); button.type = "button"; button.className = "match3-cell"; button.dataset.index = String(index); button.dataset.color = String(item.c); button.dataset.special = item.s || ""; button.textContent = item.s === "rainbow" ? "◎" : symbols[item.c];
        button.classList.toggle("is-selected", selected === index); button.disabled = locked || finished;
        button.setAttribute("role", "gridcell"); button.setAttribute("aria-label", `${rowOf(index)+1}행 ${colOf(index)+1}열 ${item.s === "rainbow" ? "컬러폭탄" : symbols[item.c] + (item.s === "h" ? " 가로 줄폭탄" : item.s === "v" ? " 세로 줄폭탄" : " 타일")}`);
    ...
    function expandSpecials(clearSet) {
      ...
      while (queue.length) {
        ...
        addSpecialClears(clearSet, index);
        if (clearSet.size > before) Array.from(clearSet).forEach(function (value) { if (!seen.has(value)) queue.push(value); });
      }
    }

    function selectSpecialAnchor(run, swapped) {
      const preferred = swapped && swapped.find(function (index) { return run.indexes.includes(index); });
      return preferred !== undefined ? preferred : run.indexes[Math.floor(run.indexes.length / 2)];
    }
    ...
    function resolveRuns(runs, chain, swapped) {
      ...
      runs.forEach(function (run) {
        ...
        if (run.indexes.length >= 5) {
          const anchor = selectSpecialAnchor(run, swapped);
          creations.set(anchor, tile(run.color, "rainbow")); clearSet.delete(anchor);
        } else if (run.indexes.length === 4) {
          const anchor = selectSpecialAnchor(run, swapped);
          creations.set(anchor, tile(run.color, run.dir)); clearSet.delete(anchor);
        }
      });
      ...
      record.bestCombo = Math.max(record.bestCombo, comboBest);
    ...
    function dropAndFill() {
      for (let c = 0; c < cols; c += 1) {
        ...
        for (let r = rows - 1; r >= 0; r -= 1) {
          const item = board[indexOf(r, c)]; if (item) stack.push(item);
        }
    ...
    function activateRainbow(a, b) {
      ...
      if (first.s === "rainbow" && second.s === "rainbow") {
        board.forEach(function (_, index) { clearSet.add(index); });
      } else {
    ...
    function reshuffle(message) {
      const specials = board.filter(function (item) { return item && item.s; }).map(function (item) { return tile(item.c, item.s); });
      board = makeBoard();
      specials.slice(0, 4).forEach(function (item) {
        const positions = board.map(function (_, index) { return index; }).sort(function () { return Math.random() - .5; });
        const index = positions.find(function (pos) { return !board[pos].s; }); if (index !== undefined) board[index] = item;
      });
      if (!validMoves(board).length || hasMatch(board)) board = makeBoard();
      selected = null;
      announce("보드 재셔플", message || "가능한 교환이 없어 자동으로 보드를 다시 섞었습니다.");
      render(0); saveGame();
    }

    function afterStable(chain, summary) {
      ...
      if (goalDone()) {
        ...
        announce("스테이지 클리어", `${stage - 1}단계를 완료했습니다. 보너스 300점과 함께 ${stage}단계를 시작합니다.`);
        render(0); saveGame(); return;
      }
      if (movesLeft <= 0) {
      ...
      announce(chain > 1 ? `연쇄 x${chain}` : "매치 성공", `${summary.removed}개 제거 · +${summary.gained}점. 이동 ${movesLeft}회 남았습니다.`);
      render(chain); saveGame();
    }

    function cascade(chain, swapped) {
      const runs = findRuns(board);
      if (!runs.length) { afterStable(Math.max(1, chain - 1), { removed: 0, gained: 0 }); return; }
      const summary = resolveRuns(runs, chain, swapped);
      render(chain);
      setTimeout(function () { cascade(chain + 1, null); }, 150);
      if (summary.removed) { /* summary is intentionally reported after the cascade settles */ }
    }
    ...
    function performSwap(a, b) {
      ...
      movesLeft -= 1;
      if (rainbow) {
        const summary = activateRainbow(a, b); render(1);
        setTimeout(function () {
          const runs = findRuns(board);
          if (runs.length) cascade(2, null);
          else { locked = false; afterStable(1, summary); }
        }, 150);
      } else {
        const firstRuns = findRuns(board);
        const summary = resolveRuns(firstRuns, 1, [a, b]); render(1);
        setTimeout(function settle() {
          const runs = findRuns(board);
          if (runs.length) {
            let chain = 2;
            function next() {
              const current = findRuns(board);
              if (!current.length) { locked = false; afterStable(chain - 1, summary); return; }
              resolveRuns(current, chain, null); render(chain); chain += 1; setTimeout(next, 150);
            }
            next();
          } else { locked = false; afterStable(1, summary); }
        }, 150);
    ...
    gameBoard.addEventListener("pointerdown", function (event) { const cell = event.target.closest(".match3-cell"); if (!cell) return; swipeStart = { x: event.clientX, y: event.clientY, index: Number(cell.dataset.index), id: event.pointerId }; try { gameBoard.setPointerCapture(event.pointerId); } catch (error) { /* optional */ } });
    ...
    const cleanupObserver = new MutationObserver(function () {
      ...
      clearTimeout(hintTimer); window.removeEventListener("keydown", onKey, true); if (externalRestart) externalRestart.removeEventListener("click", onExternalRestart, true); surface.classList.remove("match3-runtime-game"); cleanupObserver.disconnect();
    });
