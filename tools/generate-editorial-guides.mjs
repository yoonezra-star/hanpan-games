...
const guides = [
  ...
  },
  {
    ...
    description: "블록 드롭 클래식에서 오래 버티기 위한 바닥 정리, 다음 블록, 홀드, 고스트 위치와 위기 대처법을 설명합니다.",
    summary: "낙하 퍼즐은 빨리 내리는 게임처럼 보이지만, 실제 핵심은 나중에 지울 수 없는 빈칸을 만들지 않는 것입니다. 다음 블록과 홀드를 함께 읽고 보드를 낮게 유지하면 속도가 올라가도 선택지가 남습니다.",
    modifiedDate: "2026-08-20",
    readingTime: "8분",
    relatedGames: [
    ...
    sections: [
      ...
      {
        heading: "다음 블록 3개를 보고 배치 순서를 정합니다",
        paragraphs: [
          "낙하 퍼즐의 깊이는 현재 블록 하나가 아니라 다음 블록과의 연결에서 나옵니다. 한판게임에서는 다음 블록 3개가 순서대로 보이므로 현재 블록과 홀드까지 합쳐 두세 수의 배치 순서를 정할 수 있습니다. 지금 딱 맞는 자리라도 다음 두 블록의 자리를 모두 막는다면 좋은 선택이 아닐 수 있습니다.",
          "예를 들어 네모 블록이 곧 나온다면 2칸짜리 평평한 자리를 남기고, S와 Z가 연달아 나온다면 계단 모양을 서로 상쇄할 자리를 준비합니다. 모든 경우를 완벽히 계산할 필요는 없지만, 미리보기 왼쪽부터 읽고 다음 블록이 싫어할 지형을 만들지 않는 습관이 중요합니다."
        ]
      },
      {
      ...
      {
        heading: "홀드와 고스트 블록은 결정을 미리 보여 줍니다",
        paragraphs: [
          "홀드는 현재 블록이 보드 모양과 맞지 않을 때 한 번 미뤄 두는 기능입니다. 긴 막대를 통로가 준비될 때까지 보관하거나, 지금 놓으면 깊은 구멍이 생기는 블록을 다음 기회로 넘길 수 있습니다. 다만 한 블록을 떨어뜨리는 동안에는 홀드를 한 번만 사용할 수 있으므로 다음 블록까지 확인하고 결정해야 합니다.",
          "고스트 블록은 현재 블록이 떨어질 최종 위치를 반투명하게 보여 줍니다. 하드 드롭 전에 고스트가 빈칸을 덮는지, 주변 높이와 자연스럽게 이어지는지 확인하면 성급한 낙하를 줄일 수 있습니다. 작은 화면에서는 현재 블록보다 고스트 위치를 먼저 보는 편이 손가락에 가려지는 구간을 줄여 줍니다.",
          "다음 블록, 홀드, 고스트를 모두 보려고 시선을 자주 옮기면 오히려 늦어질 수 있습니다. 현재 블록의 자리를 정한 뒤 다음 블록을 확인하고, 마지막으로 고스트 위치를 검토하는 일정한 순서를 만들면 레벨이 올라가도 판단이 흔들리지 않습니다."
        ]
      },
      {
        heading: "고정 지연과 백투백은 점수를 만드는 기술입니다",
        paragraphs: [
          "블록이 바닥이나 다른 블록에 닿아도 약 0.48초 동안은 좌우 이동과 회전이 가능합니다. 고스트 위치가 한 칸 어긋났다면 이 고정 지연 안에 마지막 보정을 할 수 있습니다. 다만 한 블록에서 이동으로 지연을 계속 늘리는 횟수에는 제한이 있으므로 바닥에 닿기 전 기본 방향을 정해 두는 편이 안전합니다.",
          "누적 10줄마다 레벨이 오르고 낙하 간격이 짧아집니다. 한 번에 4줄을 지운 뒤 다시 4줄을 지우면 백투백 보너스가 붙지만, 통로 옆 높이가 위험하다면 한두 줄을 먼저 정리하는 편이 좋습니다. 콤보와 백투백은 낮은 보드를 유지한 결과로 얻어야 오래 이어집니다.",
          "모바일에서는 좌우와 내리기 버튼을 길게 눌러 연속 이동할 수 있습니다. 화면 주사율이 높은 기기에서도 실제 경과시간으로 낙하를 계산하며 다른 탭이나 앱으로 이동하면 자동으로 멈추므로, 돌아와 보드 상태를 확인한 뒤 계속 버튼을 누릅니다."
        ]
      }
    ],
    checklist: [
      ...
      "바닥에 닿기 전에 방향을 정하고 고정 지연은 마지막 보정에만 씁니다."
    ]
  },
  {
